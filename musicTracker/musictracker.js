    ...
    if (element && (e.key.length == 1)) {
        if (keycount == 0) { //Key 1 is the key, ABCDEF or G
            ...
            if (/[# -]/.test(e.key)) { //Key 2 is wether it's sharp or not (No flats supported yet)
                ...
                keycount == 0;
                nextSelected = document.querySelector('[index ="' + (parseInt(element.getAttribute('index'), 10) + 1).toString(10) + '"]')
...
function play() {
    var squareSynth = new Tone.Synth(currentSong.instruments.sq1).toDestination();
    squareSynth.data = currentSong.instruments.sq1.data
    //var triangleSynth = new Tone.Synth(currentSong.instruments.tri1).toDestination();
    //triangleSynth.data = currentSong.instruments.tri1.data
