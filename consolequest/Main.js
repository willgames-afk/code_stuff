...
/*Map Notes: All the environment data is stored in the map object. It contains a data element, which contains most of the data, 
a startText element, which contains starting text, and a directions object. The directions object contains the directions you can
move (ie, up left north south etc). For each of those direcitons, you have an x and y value that corrosponds to how much the player
will be moved and in what way. For intance, south will move you in the negative y direction. Also note that y is reversed, going
north or up should change the y value negatively, due to the way lists are indexed.

Data is stored in a matrix constructed of lists inside a list. This can be as large or small as you want. Each row stores
data from right to left (west to east). The lists do not have to be the same size but I prefer to pad them out with
falses to make each row the same. The rows are stacked on top of one another to make the map.

Every room needs a desc element, which describes the area, and an exit element, which is a list containing the room exits. 
You can also include a loot element, which is the key of an item in aI (all items object), a use element, which is a string that is the name
of the item in aI that is used to unlock the locked element.

The locked element contains a onUnlock element, which is the text displayed on unlock, and the optional move element, which is a direction in
the form of a string that will be added to the exit element on unlock.

The vis element is optional to include (the code automatically creates it on load) and is a boolean telling if the room has been visited yet.
