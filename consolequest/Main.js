var aC = { // aC for all commands
  ...
  load: 'load',
  reset: 'reset',
  exit: 'e',// Didn't like it when I put exits, so it has to be just exit. 
  map: 'map',
  downloadit: 'dload',
  loadmap: 'mapload',
  clear: 'clear',
...
function startAll() { // starts the whole thing, triggered by start or init commands
  if (!started) {
    ...
    if (!theMap.data) {
      ...
      displayText('Load a map by selecting a file using the file button over there and then typing loadmap.');
      main()
      return 'start'
    } else {
...
commands = {
  ...
  _move(direction) {
    ...
    console.log(currentCell.exit)
    console.log(aC[direction])
...
for (direction in theMap.directions) {
  function makeDir(d) { //Fancy Wrapper Function
    return function() {
      this._move(d)
    }
  }
  commands[direction] = makeDir(direction);
}
